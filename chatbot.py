# chatbot.py  –  prefix-free chat, auto-tool-picker, discord.py-self, zero dynamic code
...
PREFIX = "."   # kept for utilities only (chat is prefix-free)
...
# ---------- Prefix Commands (utilities only) ----------
def setup_chat(bot):
    async def on_ready():
...
def setup_chat(bot):
    ...
    bot.add_listener(tools_cmd, "on_message")
    bot.add_listener(fetch_cmd, "on_message")
    bot.add_listener(save_cmd, "on_message")# chatbot.py  –  prefix-free chat, auto-tool-picker, discord.py-self, zero dynamic code
import os, json, discord, redis.asyncio as redis, io
from datetime import datetime
from discord.ext import commands
from langchain_community.tools import __all__  # 1000+ pre-built tools
